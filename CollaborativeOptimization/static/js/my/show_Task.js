window.onload=function()
  ...
  $('#test_vBRAS_session_bg').click(function() {
      ...
      $.ajax('/api/v1/vnf1-uuid/',{
          ...
          id = data.taskId;
          alert(id);
          $.ajax('http://10.65.150.44:5000/TaskExec/bkg',{
              type:'POST',
              data:
                  JSON.stringify({
                      scripttype: 'itest',
                      serverIp: '10.65.104.14:8000',
                      testcase: {
                          script: [{
                              type: 'itest',
                              id: 1,
  ...
  $('#current_task_abort').click(function() {
      ...
      $.ajax('/api/v1/stop-task/',{
          ...
          data:
              JSON.stringify({
                  stop: 1,
                  taskId: '243d8bd5-15fa-48b1-b9c8-d19a2f7b7338'
              }),
      }).done(function(data){
          alert(data.current_state);
          alert(data.task_id);
          flag_id = data.task_id;
          $.ajax('http://10.65.150.44:5000/TaskExecStatus/bkg',{
          type:'POST',
          data:
              JSON.stringify({
                  action: 'abort',
